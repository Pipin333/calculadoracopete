...
def fetch_html(url, headers=None, retries=3, delay=2):
    if not headers:
        headers = get_headers()
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Access Denied (403) for URL: {url} on attempt {attempt+1}")
            else:
                print(f"Error {response.status_code} for URL: {url} on attempt {attempt+1}")
        except Exception as e:
            print(f"Request failed for URL {url}: {e} (attempt {attempt+1}/{retries})")
